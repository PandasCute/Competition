...
def feature_extraction(data_id, begin_day, end_day):
    ...
    data_id = pd.merge(data_id, activity_num, on='user_id', how='left')
    # 有activity的总天数
    activity_days = activity7.drop_duplicates(['user_id','activity_days']).groupby(['user_id'])['activity_day'].count().rename('activity7_days').reset_index()
    data_id = pd.merge(data_id, activity_days, on='user_id', how='left')
    ...
    data_id = pd.merge(data_id, activity_num, on='user_id', how='left')
    # 有activity的总天数
    activity_days = activity5.drop_duplicates(['user_id','activity_days']).groupby(['user_id'])['activity_day'].count().rename('activity5_days').reset_index()
    data_id = pd.merge(data_id, activity_days, on='user_id', how='left')
...
train = pd.concat([train1_feature, train2_feature])
train_y = train['label']
train_x = train.drop(['label','user_id'], axis=1)

test = feature_extraction(test_id, 1, 30)
...
def pred_nn():
    ...
    for i in range(5):
        ...
        model = Sequential()
        #model.add(layers.Dropout(0.2,input_shape=(33,)))

        model.add(layers.Dense(16,activation='relu',input_shape=(col,)))
    ...
    #     model.add(layers.Dropout(0.3))
        ...
        model.fit(X_train, y_train, epochs=100, batch_size=2000, verbose=2, shuffle=True)
        X_test = X_test.values
        y_prob_test = model.predict(X_test)

        score = roc_auc_score(y_test, y_prob_test)
        auc_score += score

        print('测试集 AUC：',score)
        y_prob = model.predict(test_x)

        test_id['prob'] += y_prob[:,0]
    test_id['prob'] = test_id['prob']/5
    test_id.to_csv('linxid_fuck3_nn',index=False, header=False)
    print(auc_score/5)
...
epochs=100, batch_size=1000 shape:64 32 16 8 1  0.8756452528328624
epochs=100, batch_size=1000 shape:64 32 32 16 8 1  0.8750216443005184
epochs=200, batch_size=1000 shape:64 32 16 8 1  0.8756452528328624
Dropout:0.2 0.3 0.2 0.3 
epochs=200, batch_size=1000 shape:64 32 16 8 1  0.8783397574500572
epochs=200, batch_size=1000 shape:32 16 32 16 1  0.8795687151721563
epochs=200, batch_size=500 shape:32 16 32 16 1  0.8784958644986691

epochs=200, batch_size=2000 shape:16 16 1 0.8798049452071715
"""