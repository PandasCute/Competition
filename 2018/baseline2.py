...
def get_train_label3():

    train_data_id = np.unique(pd.concat([train3_create['user_id'], train3_launch['user_id'], train3_activity['user_id']]))
    test_data_id = np.unique(pd.concat([test3_create['user_id'], test3_launch['user_id'], test3_activity['user_id']]))
...
def predcit_result():
    ...
    result['prob'] = y_prob[:,1]
    print('输出成功')
    result.to_csv('submission_linxi_0724.csv',index=False, header=False)
predcit_cv()
